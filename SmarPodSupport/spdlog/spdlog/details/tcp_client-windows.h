...
class tcp_client {
    ...
    public:
        ...
        int connect_socket_with_timeout(SOCKET sockfd, const struct sockaddr* addr, int addrlen,
                                        const timeval& tv) {
            ...
            if (::getsockopt(sockfd, SOL_SOCKET, SO_ERROR, reinterpret_cast<char*>(&so_error),
                             &len) == SOCKET_ERROR) {
                return SOCKET_ERROR;
            }
        ...
        void connect(const std::string& host, int port, int timeout_ms = 0) {
            ...
            for (auto* rp = addrinfo_result; rp != nullptr; rp = rp->ai_next) {
                ...
                if (connect_socket_with_timeout(socket_, rp->ai_addr, (int) rp->ai_addrlen, tv) ==
                    0) {
                    last_error = 0;
                    break;
                }
