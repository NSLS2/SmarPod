...
class SPDLOG_API registry {
    public:
        ...
        // set_default_logger(). e.g do not call set_default_logger() from one thread while calling
        // spdlog::info() from another.
        logger* get_default_raw();

        // set default logger and add it to the registry if not registered already.
        // default logger is stored in default_logger_ (for faster retrieval) and in the loggers_
        // map. Note: Make sure to unregister it when no longer needed or before calling again with
        // a new logger.
