...
constexpr auto get([[maybe_unused]] const T& first, [[maybe_unused]] const Args&... rest) -> const
    auto& {
...
template <typename T, typename Char>
constexpr auto parse_specs(basic_string_view<Char> str, size_t pos, int next_arg_id)
...
FMT_INLINE FMT_CONSTEXPR_STRING auto format(const CompiledFormat& cf, const T&... args)
...
template <typename OutputIt, typename CompiledFormat, typename... T,
          FMT_ENABLE_IF(detail::is_compiled_format<CompiledFormat>::value)>
constexpr FMT_INLINE auto format_to(OutputIt out, const CompiledFormat& cf, const T&... args)
    -> OutputIt {
...
#define FMT_STATIC_FORMAT(fmt_str, ...)                                                    \
    fmt::static_format_result<fmt::formatted_size(FMT_COMPILE(fmt_str), __VA_ARGS__) + 1>( \
